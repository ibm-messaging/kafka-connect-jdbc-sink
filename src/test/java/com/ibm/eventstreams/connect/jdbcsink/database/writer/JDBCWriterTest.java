...
public class JDBCWriterTest {
    ...
    public void testCreateTable_TableDoesNotExist_CreatesTable() throws SQLException {
        // Prepare test data
        String postgresCreateTableStatement = "CREATE TABLE test_table (id SERIAL PRIMARY KEY, id INTEGER NOT NULL, name VARCHAR(255) NOT NULL)";
        String db2CreateTableStatement = "CREATE TABLE test_table (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, id INTEGER NOT NULL, name VARCHAR(255) NOT NULL)";
        ...
        for (String database : Arrays.asList("POSTGRESQL", "DB2")) {
    ...
    public void testInsert_RecordsExist_InsertsRecords() throws SQLException {
        // Prepare test data
        String postgresInsertStatement = "INSERT INTO schema.test_table(id, name) VALUES (?, ?)";
        String db2InsertStatement = "INSERT INTO schema.test_table(id, name) VALUES (?, ?)";
        Map<String, String> insertStatements = new HashMap<>();
        ...
        for (String database : Arrays.asList("POSTGRESQL", "DB2")) {
            Connection connection = mock(Connection.class);
            IDataSource dataSource = spy(IDataSource.class);
            DatabaseMetaData databaseMetaData = mock(DatabaseMetaData.class);
            PreparedStatement preparedStatement = mock(PreparedStatement.class);
    ...
    public void testInsert_RecordsExist_CreateAndInsertsRecords() throws SQLException {
        // Prepare test data
        String postgresCreateTableStatement = "CREATE TABLE schema.accounts (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255), created_on BIGINT NOT NULL, last_login BIGINT NOT NULL, is_admin BOOLEAN NOT NULL, is_staff BOOLEAN NOT NULL, is_active BOOLEAN NOT NULL)";
        String db2CreateTableStatement = "CREATE TABLE schema.accounts (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id INTEGER NOT NULL, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255), created_on BIGINT NOT NULL, last_login BIGINT NOT NULL, is_admin BOOLEAN NOT NULL, is_staff BOOLEAN NOT NULL, is_active BOOLEAN NOT NULL)";
        Map<String, String> createTableStatements = new HashMap<>();
        createTableStatements.put("POSTGRESQL", postgresCreateTableStatement);
        createTableStatements.put("DB2", db2CreateTableStatement);

        String postgresInsertStatement = "INSERT INTO schema.accounts(user_id, username, password, email, created_on, last_login, is_admin, is_staff, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        String db2InsertStatement = "INSERT INTO schema.accounts(user_id, username, password, email, created_on, last_login, is_admin, is_staff, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        Map<String, String> insertStatements = new HashMap<>();
        insertStatements.put("POSTGRESQL", postgresInsertStatement);
        insertStatements.put("DB2", db2InsertStatement);

        String tableName = "schema.accounts";
        Schema schema = SchemaBuilder.struct()
        ...
        Struct johnStruct = new Struct(schema)
                .put("user_id", 1)
                .put("username", "John")
                ...
                .put("is_staff", false)
                .put("is_active", true);
        SinkRecord johnRecord = new SinkRecord("topic", 0, null, null, schema, johnStruct, 0);

        Struct janeStruct = new Struct(schema)
                .put("user_id", 2)
                .put("username", "Jane")
                ...
                .put("is_staff", true)
                .put("is_active", false);
        SinkRecord janeRecord = new SinkRecord("topic", 0, null, null, schema, janeStruct, 0);
        Collection<SinkRecord> records = Arrays.asList(johnRecord, janeRecord);
